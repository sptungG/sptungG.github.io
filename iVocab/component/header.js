...
//   for (let wordID of activeUser[0].favorite) {
//     let thisWord = words.filter((word) => word.id == wordID);
//     if (thisWord[0] != undefined) {
//       userFavorited.insertAdjacentHTML('beforeend',
//         `<li id="${wordID}" onClick="showDetailItem(this.id);">${thisWord[0].name}</li>`
//       )
//     }
//   }
// }
...
function loadUserRecipe() {
  let userRecipe = document.getElementById('user-word');
  userRecipe.innerHTML = '';
  for (let wordID of activeUser[0].created) {
    let thisWord = words.filter((word) => word.id == wordID);
    if (thisWord[0] != undefined) {
